...
def main() -> None:
    ...
    review_matches: dict[str, pd.Series] = {}
    ...
    for method_name, labels in raw_labels.items():
        ...
        method_match = mark_review(rule_name, labels, allowed)
        review_matches[method_name] = method_match
        to_review = to_review | method_match
    ...
    review_reason = pd.Series("", index=result_df.index, dtype="string")
    for method_name in review_priority:
        if method_name not in review_matches:
            continue
        method_mask = review_matches[method_name]
        review_reason = review_reason.where(~(review_reason.eq("") & method_mask), method_name)

    result_df["ReviewReason"] = review_reason
